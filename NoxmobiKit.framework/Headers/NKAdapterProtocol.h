...
/**
 ...
 @param coin_consume 适用于消耗货币的按钮；参数值(int类型)：本次点击的货币消耗量；传入-1代表不需要此参数；
 @param coin_produce_all 适用于消耗货币的按钮；参数值(int类型)：当前关卡的货币总产出量；传入-1代表不需要此参数；
 @param coin_consume_all 适用于消耗货币的按钮；参数值(int类型)：当前关卡的货币总消耗量；传入-1代表不需要此参数；
 */
